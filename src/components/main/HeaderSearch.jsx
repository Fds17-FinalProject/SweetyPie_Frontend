...
const SearchHeader = styled.form`
  ...
  background: #fff;
  width: ${({ scrollY, searchStartState }) =>
    !scrollY ? '85rem' : searchStartState ? '85rem' : '32rem'};
  height: ${({ scrollY, searchStartState }) =>
    !scrollY ? '7rem' : searchStartState ? '7rem' : '4.8rem'};
  margin-top: ${({ scrollY, searchStartState }) =>
    !scrollY ? '5rem' : searchStartState ? '5rem' : '0'};
  animation-duration: 0.1s;
  animation-timing-function: ease-out;
  animation-name: ${({ scrollY, searchStartState }) =>
    !scrollY ? scrollUp : searchStartState ? scrollUp : scrollDown};
`;

const Button = styled.button`
  display: ${({ scrollY, searchStartState }) =>
    !scrollY ? 'block' : searchStartState ? 'block' : 'none'};
  :hover {
    background: #ebebeb;
    border-radius: 999rem;
    z-index: 1;
          ...
          <div className="relative" data-name="personnel">
            ...
            {location || calendar || personnel || address.length !== 0 ? (
              <SubmitButton onClick={searchResult} data-name="no-hide">
                ...
                />
                <span
                  className="text-white text-1.6rem ml-0.4rem"
                  data-name="no-hide"
                >
                  검색
                </span>
            ...
            <LocationButton onClick={getCurrentLocation} data-name="no-hide">
              <figure data-name="no-hide">
                <img
                  ...
                  alt="어디로 여행가세요 아이콘"
                />
                <figcaption data-name="no-hide" className="a11y-hidden">
                  어디로 여행가세요?
                </figcaption>
              </figure>
              <span
                className="text-1.6rem ml-1.8rem text-#727272"
                data-name="no-hide"
              >
                가까운 여행지 둘러보기
              </span>
